...
export class Preloader extends Phaser.Scene {
    ...
    preload() {
        ...
        for (let item of Object.values(ASSETS.audio)) {

        this.load.audio(item.key, ...item.args);
        }
        // 先加载 WebFont Loader 脚本
        this.load.script(
            'webfont',
            'https://ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js'
             );

        this.load.image('yesno-panel', 'assets/ui/yesno-panel.png');
    ...
    create() {
        ...
        WebFont.load({
            custom: {
                families: ['Zombie','Minecraft','Pixelify Sans']
            },
            active: () => {
